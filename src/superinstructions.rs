...
pub fn handle_const_patterns<H: ConstHandler>(
    ...
    // Peek once to get the next instruction
    if let Some(Ok((next_op, _))) = ops.peek() {
        let next_op = next_op.clone();

        if H::supports_load_store() {
            if let Some((handler_index, memarg)) = H::try_consume_load(&next_op, ops) {
        ...
        if H::supports_load_store() {
            if let Some((handler_index, memarg)) = H::try_consume_store(&next_op, ops) {
                if let Some(operand) = H::create_memarg_operand(value, memarg) {
                    return Some(ProcessedInstr {
                        handler_index,
                        operand,
        ...
        if let Some(handler_index) = H::try_consume_comparison(&next_op, ops) {
            return Some(ProcessedInstr {
                handler_index,
        ...
        if H::supports_shift() {
            if let Some(handler_index) = H::try_consume_shift(&next_op, ops) {
                return Some(ProcessedInstr {
                    handler_index,
                    operand: H::create_value_operand(value),
                });
        ...
        if H::supports_rotation() {
            if let Some(handler_index) = H::try_consume_rotation(&next_op, ops) {
                return Some(ProcessedInstr {
                    handler_index,
                    operand: H::create_value_operand(value),
                });
        ...
        if let Some(handler_index) = H::try_consume_conversion(&next_op, ops) {
            return Some(ProcessedInstr {
                handler_index,
                operand: H::create_value_operand(value),
            });
        ...
        H::try_consume_arithmetic(&next_op, ops).map(|handler_index| ProcessedInstr {
            handler_index,
            operand: H::create_value_operand(value),
        })
    } else {
