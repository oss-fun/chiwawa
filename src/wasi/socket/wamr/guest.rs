...
use crate::wasi::socket::{arg, iovecs, read_array, write_bytes, Backend, Host};
...
pub(crate) fn open(memory: &MemAddr, params: &[Val]) -> WasiResult<()> {
    let family = decode(&FAMILY_CODES, arg(params, 1)?)?;
    let ty = decode(&TYPE_CODES, arg(params, 2)?)?;
    let fd = Host::open(family, ty)?;
    write_bytes(memory, arg(params, 3)?, &fd.to_le_bytes())
}

/// `sock_bind(fd, addr)`
pub(crate) fn bind(memory: &MemAddr, params: &[Val]) -> WasiResult<()> {
    let addr = decode_addr(&read_array(memory, arg(params, 1)?)?)?;
    Host::bind(arg(params, 0)?, &addr)
}

/// `sock_connect(fd, addr)`
pub(crate) fn connect(memory: &MemAddr, params: &[Val]) -> WasiResult<()> {
    let addr = decode_addr(&read_array(memory, arg(params, 1)?)?)?;
    Host::connect(arg(params, 0)?, &addr)
}

/// `sock_addr_local(fd, addr_out)`
pub(crate) fn addr_local(memory: &MemAddr, params: &[Val]) -> WasiResult<()> {
    let addr = Host::local_addr(arg(params, 0)?)?;
    write_bytes(memory, arg(params, 1)?, &encode_addr(&addr))
}

/// `sock_addr_remote(fd, addr_out)`
pub(crate) fn addr_remote(memory: &MemAddr, params: &[Val]) -> WasiResult<()> {
    let addr = Host::peer_addr(arg(params, 0)?)?;
    write_bytes(memory, arg(params, 1)?, &encode_addr(&addr))
}
...
pub(crate) fn recv_from(memory: &MemAddr, params: &[Val]) -> WasiResult<()> {
    ...
    let (len, from) = Host::recv_from(arg(params, 0)?, &iovecs, arg(params, 3)? as u32)?;
    write_bytes(memory, arg(params, 4)?, &encode_addr(&from))?;
    write_bytes(memory, arg(params, 5)?, &len.to_le_bytes())
}

/// `sock_send_to(fd, iovs, iovs_len, flags, addr, len_out)`
pub(crate) fn send_to(memory: &MemAddr, params: &[Val]) -> WasiResult<()> {
    let to = decode_addr(&read_array(memory, arg(params, 4)?)?)?;
    let iovecs = iovecs(memory, params)?;
    let len = Host::send_to(arg(params, 0)?, &iovecs, arg(params, 3)? as u32, &to)?;
    write_bytes(memory, arg(params, 5)?, &len.to_le_bytes())
}

/// `sock_close(fd)`
pub(crate) fn close(params: &[Val]) -> WasiResult<()> {
    Host::close(arg(params, 0)?)
}
