...
mod tests {
    ...
    fn test_path_rename() {
        ...
        let args = vec![
            "path_rename.wasm".to_string(), // argv[0] - program name
            scratch_dir.to_string(),        // argv[1] - scratch directory
        ];

        let inst = load_wasi_instance_with_args("tests/wasi/path_rename.wasm", args);
        let result = run_wasi_module(&inst);

        result.expect("path_rename should succeed");
    }
}
