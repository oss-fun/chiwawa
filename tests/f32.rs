...
mod tests {
    ...
    fn test_f32_add() {
        ...
        assert_f32_eq(ret.unwrap().last().unwrap().to_f32().unwrap(), f32::INFINITY);

        let ret = call_function(
            &inst,
            "add",
            vec![Val::Num(Num::F32(f32::NEG_INFINITY)), Val::Num(Num::F32(1.0))],
        );
        assert_f32_eq(ret.unwrap().last().unwrap().to_f32().unwrap(), f32::NEG_INFINITY);
        ...
        let ret = call_function(
            &inst,
            "add",
            vec![Val::Num(Num::F32(f32::INFINITY)), Val::Num(Num::F32(f32::NEG_INFINITY))],
        );
        assert!(ret.unwrap().last().unwrap().to_f32().unwrap().is_nan());
        ...
        let ret = call_function(
            &inst,
            "add",
            vec![Val::Num(Num::F32(min_positive)), Val::Num(Num::F32(min_positive))],
        );
        assert_f32_eq(ret.unwrap().last().unwrap().to_f32().unwrap(), min_positive * 2.0);
    }
    ...
    fn test_f32_sub() {
        ...
        let ret = call_function(
            &inst,
            "sub",
            vec![Val::Num(Num::F32(f32::INFINITY)), Val::Num(Num::F32(f32::INFINITY))],
        );
        assert!(ret.unwrap().last().unwrap().to_f32().unwrap().is_nan());
    ...
    fn test_f32_special_arithmetic() {
        ...
        let ret = call_function(
            &inst,
            "div",
            vec![Val::Num(Num::F32(f32::MIN_POSITIVE)), Val::Num(Num::F32(2.0))],
        );
        assert_f32_eq(ret.unwrap().last().unwrap().to_f32().unwrap(), f32::MIN_POSITIVE / 2.0);

        // Test multiplication with very small numbers
        let ret = call_function(
            &inst,
            "mul",
            vec![Val::Num(Num::F32(f32::MIN_POSITIVE)), Val::Num(Num::F32(0.5))],
        );
        let result = ret.unwrap().last().unwrap().to_f32().unwrap();
    ...
    fn test_f32_spec_hex_values() {
        ...
        let ret = call_function(&inst, "add", vec![Val::Num(Num::F32(val_1p149)), Val::Num(Num::F32(val_1p149))]);
        ...
        let ret = call_function(&inst, "add", vec![Val::Num(Num::F32(0.0)), Val::Num(Num::F32(val_1p126))]);
        ...
        let ret = call_function(&inst, "mul", vec![Val::Num(Num::F32(val_pi)), Val::Num(Num::F32(1.0))]);
        ...
        let ret = call_function(&inst, "add", vec![Val::Num(Num::F32(val_max)), Val::Num(Num::F32(0.0))]);
    ...
    fn test_f32_spec_overflow() {
        ...
        let ret = call_function(&inst, "add", vec![Val::Num(Num::F32(f32::MAX)), Val::Num(Num::F32(f32::MAX))]);
        assert_f32_eq(ret.unwrap().last().unwrap().to_f32().unwrap(), f32::INFINITY);
    ...
    fn test_f32_spec_underflow() {
        ...
        let ret = call_function(&inst, "div", vec![Val::Num(Num::F32(tiny)), Val::Num(Num::F32(f32::MAX))]);
        let result = ret.unwrap().last().unwrap().to_f32().unwrap();
    ...
    fn test_f32_spec_canonical_nan_arithmetic() {
        ...
        let ret = call_function(&inst, "add", vec![Val::Num(Num::F32(canonical_nan)), Val::Num(Num::F32(1.0))]);
        assert!(ret.unwrap().last().unwrap().to_f32().unwrap().is_nan());
        
        let ret = call_function(&inst, "sub", vec![Val::Num(Num::F32(arithmetic_nan)), Val::Num(Num::F32(1.0))]);
        assert!(ret.unwrap().last().unwrap().to_f32().unwrap().is_nan());
        
        let ret = call_function(&inst, "mul", vec![Val::Num(Num::F32(canonical_nan)), Val::Num(Num::F32(1.0))]);
        assert!(ret.unwrap().last().unwrap().to_f32().unwrap().is_nan());
        
        let ret = call_function(&inst, "div", vec![Val::Num(Num::F32(arithmetic_nan)), Val::Num(Num::F32(1.0))]);
        assert!(ret.unwrap().last().unwrap().to_f32().unwrap().is_nan());
    }
    ...
    fn test_f32_spec_precise_zero_signs() {
        ...
        let ret = call_function(&inst, "add", vec![Val::Num(Num::F32(-0.0)), Val::Num(Num::F32(-0.0))]);
        let result = ret.unwrap().last().unwrap().to_f32().unwrap();
        assert_eq!(result.to_bits(), (-0.0f32).to_bits());
        
        let ret = call_function(&inst, "sub", vec![Val::Num(Num::F32(0.0)), Val::Num(Num::F32(0.0))]);
        let result = ret.unwrap().last().unwrap().to_f32().unwrap();
        assert_eq!(result.to_bits(), 0.0f32.to_bits());
        
        let ret = call_function(&inst, "mul", vec![Val::Num(Num::F32(-0.0)), Val::Num(Num::F32(1.0))]);
        let result = ret.unwrap().last().unwrap().to_f32().unwrap();
        assert_eq!(result.to_bits(), (-0.0f32).to_bits());
    }
    ...
    fn test_f32_spec_subnormal_arithmetic() {
        ...
        // smallest - smallest = 0
        let ret = call_function(&inst, "sub", vec![Val::Num(Num::F32(smallest)), Val::Num(Num::F32(smallest))]);
        assert_f32_eq(ret.unwrap().last().unwrap().to_f32().unwrap(), 0.0);
        
        // smallest + smallest = second_smallest
        let ret = call_function(&inst, "add", vec![Val::Num(Num::F32(smallest)), Val::Num(Num::F32(smallest))]);
        assert_f32_eq(ret.unwrap().last().unwrap().to_f32().unwrap(), second_smallest);
    ...
    fn test_f32_spec_min_max_precise() {
        ...
        let ret = call_function(&inst, "min", vec![Val::Num(Num::F32(0.0)), Val::Num(Num::F32(-0.0))]);
        let result = ret.unwrap().last().unwrap().to_f32().unwrap();
        assert_eq!(result.to_bits(), (-0.0f32).to_bits()); 
        
        let ret = call_function(&inst, "max", vec![Val::Num(Num::F32(0.0)), Val::Num(Num::F32(-0.0))]);
        let result = ret.unwrap().last().unwrap().to_f32().unwrap();
        assert_eq!(result.to_bits(), 0.0f32.to_bits());
        
        let tiny = f32::from_bits(0x00000001);
        let ret = call_function(&inst, "min", vec![Val::Num(Num::F32(tiny)), Val::Num(Num::F32(0.0))]);
        assert_f32_eq(ret.unwrap().last().unwrap().to_f32().unwrap(), 0.0);
        
        let ret = call_function(&inst, "max", vec![Val::Num(Num::F32(tiny)), Val::Num(Num::F32(0.0))]);
