...
mod tests {
    ...
    fn call_function(inst: &Arc<ModuleInst>, func_name: &str, params: Vec<Val>) -> Result<Vec<Val>, chiwawa::error::RuntimeError> {
     ...
     fn test_call_indirect_type_all_i32_i64() {
         let inst = load_instance("test/call_indirect.wasm");
         let ret = call_function(&inst, "type-all-i32-i64", vec![])
             .unwrap();
         assert_eq!(ret.len(), 2);
     ...
     fn test_call_indirect_dispatch() {
         let inst = load_instance("test/call_indirect.wasm");
         let ret = call_function(&inst, "dispatch", vec![Val::Num(Num::I32(5)), Val::Num(Num::I64(64))]);
         assert_eq!(ret.unwrap().last().unwrap().to_i64().unwrap(), 64);
         let ret = call_function(&inst, "dispatch", vec![Val::Num(Num::I32(5)), Val::Num(Num::I64(123))]);
     ...
     fn test_call_indirect_dispatch_structural_i64() {
         let inst = load_instance("test/call_indirect.wasm");
         let ret = call_function(&inst, "dispatch-structural-i64", vec![Val::Num(Num::I32(20))]);
     ...
     fn test_call_indirect_dispatch_structural_i32() {
         let inst = load_instance("test/call_indirect.wasm");
         let ret = call_function(&inst, "dispatch-structural-i32", vec![Val::Num(Num::I32(19))]);
     ...
     fn test_call_indirect_dispatch_structural_f32() {
         let inst = load_instance("test/call_indirect.wasm");
         let ret = call_function(&inst, "dispatch-structural-f32", vec![Val::Num(Num::I32(21))]);
         assert_eq!(
             ret.unwrap().last().unwrap().to_f32().unwrap().to_bits(),
             9.0_f32.to_bits()
         );
     ...
     fn test_call_indirect_dispatch_structural_f64() {
         let inst = load_instance("test/call_indirect.wasm");
         let ret = call_function(&inst, "dispatch-structural-f64", vec![Val::Num(Num::I32(22))]);
         assert_eq!(
             ret.unwrap().last().unwrap().to_f64().unwrap().to_bits(),
             9.0_f64.to_bits()
         );
