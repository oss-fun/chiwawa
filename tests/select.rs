...
mod tests {
    ...
    fn test_select_i32_spec() {
        ...
        let ret = call_function(&inst, "select-i32", vec![Val::Num(Num::I32(1)), Val::Num(Num::I32(2)), Val::Num(Num::I32(1))]);
        ...
        let ret = call_function(&inst, "select-i32", vec![Val::Num(Num::I32(1)), Val::Num(Num::I32(2)), Val::Num(Num::I32(0))]);
        ...
        let ret = call_function(&inst, "select-i32", vec![Val::Num(Num::I32(2)), Val::Num(Num::I32(1)), Val::Num(Num::I32(0))]);
        assert_eq!(ret.unwrap().last().unwrap().to_i32().unwrap(), 1);
    }
    ...
    fn test_select_i64_spec() {
        ...
        let ret = call_function(&inst, "select-i64", vec![Val::Num(Num::I64(2)), Val::Num(Num::I64(1)), Val::Num(Num::I32(1))]);
        ...
        let ret = call_function(&inst, "select-i64", vec![Val::Num(Num::I64(2)), Val::Num(Num::I64(1)), Val::Num(Num::I32(0xf0f0f0f0u32 as i32))]);
        assert_eq!(ret.unwrap().last().unwrap().to_i64().unwrap(), 2);
    }
    ...
    fn test_select_f32_spec() {
        ...
        let ret = call_function(&inst, "select-f32", vec![Val::Num(Num::F32(1.0)), Val::Num(Num::F32(2.0)), Val::Num(Num::I32(1))]);
        ...
        let ret = call_function(&inst, "select-f32", vec![Val::Num(Num::F32(f32::NAN)), Val::Num(Num::F32(1.0)), Val::Num(Num::I32(1))]);
        ...
        let ret = call_function(&inst, "select-f32", vec![Val::Num(Num::F32(f32::from_bits(0x7fc20304))), Val::Num(Num::F32(1.0)), Val::Num(Num::I32(1))]);
        assert!(ret.unwrap().last().unwrap().to_f32().unwrap().is_nan());

        // (assert_return (invoke "select-f32" (f32.const nan) (f32.const 1) (i32.const 0)) (f32.const 1))
        let ret = call_function(&inst, "select-f32", vec![Val::Num(Num::F32(f32::NAN)), Val::Num(Num::F32(1.0)), Val::Num(Num::I32(0))]);
        assert_eq!(ret.unwrap().last().unwrap().to_f32().unwrap(), 1.0);

        // (assert_return (invoke "select-f32" (f32.const nan:0x20304) (f32.const 1) (i32.const 0)) (f32.const 1))
        let ret = call_function(&inst, "select-f32", vec![Val::Num(Num::F32(f32::from_bits(0x7fc20304))), Val::Num(Num::F32(1.0)), Val::Num(Num::I32(0))]);
        assert_eq!(ret.unwrap().last().unwrap().to_f32().unwrap(), 1.0);

        // (assert_return (invoke "select-f32" (f32.const 2) (f32.const nan) (i32.const 1)) (f32.const 2))
        let ret = call_function(&inst, "select-f32", vec![Val::Num(Num::F32(2.0)), Val::Num(Num::F32(f32::NAN)), Val::Num(Num::I32(1))]);
        ...
        let ret = call_function(&inst, "select-f32", vec![Val::Num(Num::F32(2.0)), Val::Num(Num::F32(f32::from_bits(0x7fc20304))), Val::Num(Num::I32(1))]);
        assert_eq!(ret.unwrap().last().unwrap().to_f32().unwrap(), 2.0);

        // (assert_return (invoke "select-f32" (f32.const 2) (f32.const nan) (i32.const 0)) (f32.const nan))
        let ret = call_function(&inst, "select-f32", vec![Val::Num(Num::F32(2.0)), Val::Num(Num::F32(f32::NAN)), Val::Num(Num::I32(0))]);
        assert!(ret.unwrap().last().unwrap().to_f32().unwrap().is_nan());

        // (assert_return (invoke "select-f32" (f32.const 2) (f32.const nan:0x20304) (i32.const 0)) (f32.const nan:0x20304))
        let ret = call_function(&inst, "select-f32", vec![Val::Num(Num::F32(2.0)), Val::Num(Num::F32(f32::from_bits(0x7fc20304))), Val::Num(Num::I32(0))]);
        assert!(ret.unwrap().last().unwrap().to_f32().unwrap().is_nan());
    }
    ...
    fn test_select_f64_spec() {
        ...
        let ret = call_function(&inst, "select-f64", vec![Val::Num(Num::F64(1.0)), Val::Num(Num::F64(2.0)), Val::Num(Num::I32(1))]);
        ...
        let ret = call_function(&inst, "select-f64", vec![Val::Num(Num::F64(f64::NAN)), Val::Num(Num::F64(1.0)), Val::Num(Num::I32(1))]);
        ...
        let ret = call_function(&inst, "select-f64", vec![Val::Num(Num::F64(f64::from_bits(0x7ff8400203040000))), Val::Num(Num::F64(1.0)), Val::Num(Num::I32(1))]);
        assert!(ret.unwrap().last().unwrap().to_f64().unwrap().is_nan());

        // (assert_return (invoke "select-f64" (f64.const nan) (f64.const 1) (i32.const 0)) (f64.const 1))
        let ret = call_function(&inst, "select-f64", vec![Val::Num(Num::F64(f64::NAN)), Val::Num(Num::F64(1.0)), Val::Num(Num::I32(0))]);
        assert_eq!(ret.unwrap().last().unwrap().to_f64().unwrap(), 1.0);

        // (assert_return (invoke "select-f64" (f64.const nan:0x4020304) (f64.const 1) (i32.const 0)) (f64.const 1))
        let ret = call_function(&inst, "select-f64", vec![Val::Num(Num::F64(f64::from_bits(0x7ff8400203040000))), Val::Num(Num::F64(1.0)), Val::Num(Num::I32(0))]);
        assert_eq!(ret.unwrap().last().unwrap().to_f64().unwrap(), 1.0);

        // (assert_return (invoke "select-f64" (f64.const 2) (f64.const nan) (i32.const 1)) (f64.const 2))
        let ret = call_function(&inst, "select-f64", vec![Val::Num(Num::F64(2.0)), Val::Num(Num::F64(f64::NAN)), Val::Num(Num::I32(1))]);
        ...
        let ret = call_function(&inst, "select-f64", vec![Val::Num(Num::F64(2.0)), Val::Num(Num::F64(f64::from_bits(0x7ff8400203040000))), Val::Num(Num::I32(1))]);
        assert_eq!(ret.unwrap().last().unwrap().to_f64().unwrap(), 2.0);

        // (assert_return (invoke "select-f64" (f64.const 2) (f64.const nan) (i32.const 0)) (f64.const nan))
        let ret = call_function(&inst, "select-f64", vec![Val::Num(Num::F64(2.0)), Val::Num(Num::F64(f64::NAN)), Val::Num(Num::I32(0))]);
        assert!(ret.unwrap().last().unwrap().to_f64().unwrap().is_nan());

        // (assert_return (invoke "select-f64" (f64.const 2) (f64.const nan:0x4020304) (i32.const 0)) (f64.const nan:0x4020304))
        let ret = call_function(&inst, "select-f64", vec![Val::Num(Num::F64(2.0)), Val::Num(Num::F64(f64::from_bits(0x7ff8400203040000))), Val::Num(Num::I32(0))]);
        assert!(ret.unwrap().last().unwrap().to_f64().unwrap().is_nan());
    }
    ...
    fn test_select_typed_spec() {
        ...
        let ret = call_function(&inst, "select-i32-t", vec![Val::Num(Num::I32(1)), Val::Num(Num::I32(2)), Val::Num(Num::I32(1))]);
        assert_eq!(ret.unwrap().last().unwrap().to_i32().unwrap(), 1);
        //(assert_return (invoke "select-i32-t" (i32.const 1) (i32.const 2) (i32.const 0)) (i32.const 2))
        let ret = call_function(&inst, "select-i32-t", vec![Val::Num(Num::I32(1)), Val::Num(Num::I32(2)), Val::Num(Num::I32(0))]);
        assert_eq!(ret.unwrap().last().unwrap().to_i32().unwrap(), 2);
        // (assert_return (invoke "select-i32-t" (i32.const 2) (i32.const 1) (i32.const 0)) (i32.const 1))
        let ret = call_function(&inst, "select-i32-t", vec![Val::Num(Num::I32(2)), Val::Num(Num::I32(1)), Val::Num(Num::I32(0))]);
        assert_eq!(ret.unwrap().last().unwrap().to_i32().unwrap(), 1);

        // (assert_return (invoke "select-i64-t" (i64.const 2) (i64.const 1) (i32.const 1)) (i64.const 2))
        let ret = call_function(&inst, "select-i64-t", vec![Val::Num(Num::I64(2)), Val::Num(Num::I64(1)), Val::Num(Num::I32(1))]);
        assert_eq!(ret.unwrap().last().unwrap().to_i64().unwrap(), 2);
        //(assert_return (invoke "select-i64-t" (i64.const 2) (i64.const 1) (i32.const -1)) (i64.const 2))
        let ret = call_function(&inst, "select-i64-t", vec![Val::Num(Num::I64(2)), Val::Num(Num::I64(1)), Val::Num(Num::I32(-1))]);
        assert_eq!(ret.unwrap().last().unwrap().to_i64().unwrap(), 2);
        //(assert_return (invoke "select-i64-t" (i64.const 2) (i64.const 1) (i32.const 0xf0f0f0f0)) (i64.const 2))
        let ret = call_function(&inst, "select-i64-t", vec![Val::Num(Num::I64(2)), Val::Num(Num::I64(1)), Val::Num(Num::I32(0xf0f0f0f0u32 as i32))]);
        assert_eq!(ret.unwrap().last().unwrap().to_i64().unwrap(), 2);

        // (assert_return (invoke "select-f32-t" (f32.const nan) (f32.const 1) (i32.const 1)) (f32.const nan))
        let ret = call_function(&inst, "select-f32-t", vec![Val::Num(Num::F32(f32::NAN)), Val::Num(Num::F32(1.0)), Val::Num(Num::I32(1))]);
        assert!(ret.unwrap().last().unwrap().to_f32().unwrap().is_nan());

        // (assert_return (invoke "select-f32-t" (f32.const nan:0x20304) (f32.const 1) (i32.const 1)) (f32.const nan:0x20304))
        let ret = call_function(&inst, "select-f32-t", vec![Val::Num(Num::F32(f32::from_bits(0x7fc20304))), Val::Num(Num::F32(1.0)), Val::Num(Num::I32(1))]);
        assert!(ret.unwrap().last().unwrap().to_f32().unwrap().is_nan());

        // (assert_return (invoke "select-f32-t" (f32.const nan) (f32.const 1) (i32.const 0)) (f32.const 1))
        let ret = call_function(&inst, "select-f32-t", vec![Val::Num(Num::F32(f32::NAN)), Val::Num(Num::F32(1.0)), Val::Num(Num::I32(0))]);
        assert_eq!(ret.unwrap().last().unwrap().to_f32().unwrap(), 1.0);

        // (assert_return (invoke "select-f32-t" (f32.const nan:0x20304) (f32.const 1) (i32.const 0)) (f32.const 1))
        let ret = call_function(&inst, "select-f32-t", vec![Val::Num(Num::F32(f32::from_bits(0x7fc20304))), Val::Num(Num::F32(1.0)), Val::Num(Num::I32(0))]);
        assert_eq!(ret.unwrap().last().unwrap().to_f32().unwrap(), 1.0);

        // (assert_return (invoke "select-f32-t" (f32.const 2) (f32.const nan) (i32.const 1)) (f32.const 2))
        let ret = call_function(&inst, "select-f32-t", vec![Val::Num(Num::F32(2.0)), Val::Num(Num::F32(f32::NAN)), Val::Num(Num::I32(1))]);
        assert_eq!(ret.unwrap().last().unwrap().to_f32().unwrap(), 2.0);

        // (assert_return (invoke "select-f32-t" (f32.const 2) (f32.const nan:0x20304) (i32.const 1)) (f32.const 2))
        let ret = call_function(&inst, "select-f32-t", vec![Val::Num(Num::F32(2.0)), Val::Num(Num::F32(f32::from_bits(0x7fc20304))), Val::Num(Num::I32(1))]);
        assert_eq!(ret.unwrap().last().unwrap().to_f32().unwrap(), 2.0);

        // (assert_return (invoke "select-f32-t" (f32.const 2) (f32.const nan) (i32.const 0)) (f32.const nan))
        let ret = call_function(&inst, "select-f32-t", vec![Val::Num(Num::F32(2.0)), Val::Num(Num::F32(f32::NAN)), Val::Num(Num::I32(0))]);
        assert!(ret.unwrap().last().unwrap().to_f32().unwrap().is_nan());

        // (assert_return (invoke "select-f32-t" (f32.const 2) (f32.const nan:0x20304) (i32.const 0)) (f32.const nan:0x20304))
        let ret = call_function(&inst, "select-f32-t", vec![Val::Num(Num::F32(2.0)), Val::Num(Num::F32(f32::from_bits(0x7fc20304))), Val::Num(Num::I32(0))]);
        assert!(ret.unwrap().last().unwrap().to_f32().unwrap().is_nan());

        // (assert_return (invoke "select-f64-t" (f64.const nan) (f64.const 1) (i32.const 1)) (f64.const nan))
        let ret = call_function(&inst, "select-f64-t", vec![Val::Num(Num::F64(f64::NAN)), Val::Num(Num::F64(1.0)), Val::Num(Num::I32(1))]);
        assert!(ret.unwrap().last().unwrap().to_f64().unwrap().is_nan());

        // (assert_return (invoke "select-f64-t" (f64.const nan:0x4020304) (f64.const 1) (i32.const 1)) (f64.const nan:0x4020304))
        let ret = call_function(&inst, "select-f64-t", vec![Val::Num(Num::F64(f64::from_bits(0x7ff8400203040000))), Val::Num(Num::F64(1.0)), Val::Num(Num::I32(1))]);
        assert!(ret.unwrap().last().unwrap().to_f64().unwrap().is_nan());

        // (assert_return (invoke "select-f64-t" (f64.const nan) (f64.const 1) (i32.const 0)) (f64.const 1))
        let ret = call_function(&inst, "select-f64-t", vec![Val::Num(Num::F64(f64::NAN)), Val::Num(Num::F64(1.0)), Val::Num(Num::I32(0))]);
        assert_eq!(ret.unwrap().last().unwrap().to_f64().unwrap(), 1.0);

        // (assert_return (invoke "select-f64-t" (f64.const nan:0x4020304) (f64.const 1) (i32.const 0)) (f64.const 1))
        let ret = call_function(&inst, "select-f64-t", vec![Val::Num(Num::F64(f64::from_bits(0x7ff8400203040000))), Val::Num(Num::F64(1.0)), Val::Num(Num::I32(0))]);
        assert_eq!(ret.unwrap().last().unwrap().to_f64().unwrap(), 1.0);

        // (assert_return (invoke "select-f64-t" (f64.const 2) (f64.const nan) (i32.const 1)) (f64.const 2))
        let ret = call_function(&inst, "select-f64-t", vec![Val::Num(Num::F64(2.0)), Val::Num(Num::F64(f64::NAN)), Val::Num(Num::I32(1))]);
        assert_eq!(ret.unwrap().last().unwrap().to_f64().unwrap(), 2.0);

        // (assert_return (invoke "select-f64-t" (f64.const 2) (f64.const nan:0x4020304) (i32.const 1)) (f64.const 2))
        let ret = call_function(&inst, "select-f64-t", vec![Val::Num(Num::F64(2.0)), Val::Num(Num::F64(f64::from_bits(0x7ff8400203040000))), Val::Num(Num::I32(1))]);
        assert_eq!(ret.unwrap().last().unwrap().to_f64().unwrap(), 2.0);

        // (assert_return (invoke "select-f64-t" (f64.const 2) (f64.const nan) (i32.const 0)) (f64.const nan))
        let ret = call_function(&inst, "select-f64-t", vec![Val::Num(Num::F64(2.0)), Val::Num(Num::F64(f64::NAN)), Val::Num(Num::I32(0))]);
        assert!(ret.unwrap().last().unwrap().to_f64().unwrap().is_nan());

        // (assert_return (invoke "select-f64-t" (f64.const 2) (f64.const nan:0x4020304) (i32.const 0)) (f64.const nan:0x4020304))
        let ret = call_function(&inst, "select-f64-t", vec![Val::Num(Num::F64(2.0)), Val::Num(Num::F64(f64::from_bits(0x7ff8400203040000))), Val::Num(Num::I32(0))]);
        assert!(ret.unwrap().last().unwrap().to_f64().unwrap().is_nan());
