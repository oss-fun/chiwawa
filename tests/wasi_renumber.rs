...
mod tests {
    ...
    fn run_wasi_module(inst: &Rc<ModuleInst>) -> Result<Vec<Val>, chiwawa::error::RuntimeError> {
        ...
        let mut runtime = Runtime::new(Rc::clone(inst), &func_addr, vec![], true, false)?;
        runtime.run()
    }
    ...
    #[cfg_attr(
        feature = "wasmtime",
        ignore = "fd_renumber validation differs on wasmtime"
    )]
    fn test_renumber() {
        let scratch_dir = "tests/testdir";

        let args = vec![
            "renumber.wasm".to_string(), // argv[0] - program name
            scratch_dir.to_string(),     // argv[1] - scratch directory
        ];

        let inst = load_wasi_instance_with_args("tests/wasi/renumber.wasm", args);
        let result = run_wasi_module(&inst);

        let cleanup_dir = format!("{}/renumber_dir.cleanup", scratch_dir);
        if std::path::Path::new(&cleanup_dir).exists() {
            std::fs::remove_dir_all(&cleanup_dir).ok();
        }

        result.expect("renumber should succeed");
    }
}
